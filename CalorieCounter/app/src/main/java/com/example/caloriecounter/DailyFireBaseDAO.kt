...
class DailyFireBaseDAO:DailyDAO {
    ...
    override fun insertCalories(std: Daily) {
        ...
        val day =
            Daily(std.currentdate, std.currentday, std.currentmonth, std.foodname, std.calories)
    ...
    override fun readCalories(iD: Int?,month:String?, callback: (conversationList: MutableList<Daily>) -> Unit) {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(snapshot.exists()) {
                    for (i in snapshot.children) {
                        ...
                        if (currentdate == iD && currentmonth==month) {
    ...
    override fun UpdateCalories(iD: Int?, callback:  (Int) -> Unit) {
        var sum: Double=0.0
